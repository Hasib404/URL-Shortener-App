...
async def update_product(product_update_req: ProductUpdateRequest):
    ...
    try:
        is_product_updated = session.query(Product).filter(Product.id == product_id).update({
            Product.name: product_update_req.name, 
            Product.price: product_update_req.price,
...
async def delete_product(product_id: str):
    session = database.get_db_session(engine)
    try:
        is_product_updated = session.query(Product).filter(Product.id == product_id).delete(synchronize_session=False)
        session.flush()
        session.commit()
...
async def read_product(product_id: str):
    ...
    try:
        data = session.query(Product).filter(Product.id == product_id).one()
    except Exception as ex:
        print("Error", ex)
